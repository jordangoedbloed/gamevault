    ...
    public function store(Request $request, Game $game)
{
    ...
    // Admins mogen altijd een review plaatsen (bypass diepere validatie)
    if (auth()->user()->role !== 'admin') {
        ...
        if ($distinctDays < 5) {
            return back()
                ->withErrors([
                    'body' => 'Je kunt pas een review plaatsen na minimaal 5 verschillende speeldagen. (Nu: '.$distinctDays.'/5)'
                ])
                ->withInput();
        }
    }
